...
(async () => {
	try {
		...
		const source = await readFile('./dist/qb-field.js');

		searchStr = 'const quickbase_1 =';
		searchRgx = new RegExp(searchStr);

		await writeFile('./dist/qb-field.prep.js', source.toString().replace(searchRgx, [
		...
		await writeFile('./dist/qb-field.browserify.js', browserified.outputText);

		console.log('Minify...');
		const results = await minify('./dist/qb-field.browserify.js');
		const license = await readFile('./LICENSE');

		searchStr = '1\\:\\[function\\(e,t,n\\)\\{\\(function\\(t\\)\\{\\"use strict\\";Object\\.defineProperty\\(n,\\"__esModule\\",\\{value\\:\\!0\\}\\);';
		searchRgx = new RegExp(searchStr);

		await writeFile('./dist/qb-field.browserify.min.js', results.toString().replace(searchRgx, [
		...
		console.log('Cleanup...');
		await exec([
			'rm ./dist/qb-field.prep.js',
			'rm ./dist/qb-field.browserify.js'
		].join(' && '));
